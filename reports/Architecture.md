...
![ C4 - Level 1: System Context ](../analysis/architecture/images/systemContext_diagram.svg)
...
![ C4 - Level 2: Container Diagram ](../analysis/architecture/images/container_diagram.svg)
...
However, one important Clean Architecture principle is still present: dependencies point toward the central, reusable core. This was verified with an "include-level" dependency analysis. The `Samples`, `JoltViewer`, `PerformanceTest`, `UnitTests` containers depend on the Jolt Physics Library, while no include dependency is visible from the library to those supporting containers.
...
![ C4 - Level 3: Component Diagram ](../analysis/architecture/images/ComponentDetailedView.svg)

### Detailed Component Interactions:

![ C4 - Level 3: Component Detailed Diagram ](../analysis/architecture/images/ComponentView.svg)

### UML Component Diagram:

![ UML Component Diagram ](../analysis/architecture/images/ComponentDiagramUML.png)
...
| # | Class              | File                 | Issue                                                                                                                                                                                                                                                                                                                                                                            |
| --- | -------------------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | `CharacterVirtual` | `CharacterVirtual.h` | This class moves a character, but it also records every contact point the character touches, resolves penetration (when the character clips into another shape), detects what ground the character is standing on, and predicts future contacts before they happen (five different responsabilities).        |
| 2 | `RagdollSettings`  | `Ragdoll.h`          | This class should just hold configuration values, but this also maps the ragdoll onto a skeleton, saves and loads itself (serialization), and contains logic to stabilize the ragdoll simulation (four different responsabilities).                                                        |
...
| # | Class               | File                  | Issue                                                                                                                                                                                                                                                                                                                  |
| --- | --------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1 | `CollisionDispatch` | `CollisionDispatch.h` | When adding a new shape type manual registration is required. When creating a new shape, `CollisionDispatch` must be edited to register the new shape and how it collides with every other shape.                                                               |
| 2 | `EShapeSubType`     | `Shape.h`             | When adding a new shape type the enum needs to be modified, which forces every file that uses the list to recompile (similarly to the previous violation). |
...
| # | Class              | File                 | Issue                                                                                                                                                                    |
| --- | -------------------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | `CharacterVirtual` | `CharacterVirtual.h` | Method `GetBodyID()` always returns invalid: `CharacterVirtual` inherits from `CharacterBase`, which has a method `GetBodyID()` that should return the physics body attached to the character, but `CharacterVirtual` has no physics body to move so `GetBodyID()` always returns an invalid ID. Any code that calls `GetBodyID()` of a `CharacterBase` and then uses the result will silently fail if it receives a `CharacterVirtual` |
...
| # | Class           | File              | Issue                                                                                                                                                                                                                                                                                         |
| --- | ----------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | `Shape`         | `Shape.h`         | The `Shape` base class forces any child-class to provide methods for collision detection, geometry queries, mass calculation, debug rendering, serialization. A class that only needs to render a shape for debug purposes is still forced to implement collision detection methods it will never use.                                                                     |
| 2 | `BodyInterface` | `BodyInterface.h` | It handles creation, transform, velocity, forces, activation, query, material. `BodyInterface` is the main way to interact with physics bodies but in a system that only needs to query positions each body still depends on the base interface including method (i.e. force application ones) it will never call. |
...
| # | Class                      | File                         | Issue                                                                                                                                                                                                                                                                                                                                                                                      |
| --- | ---------------------------- | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | `RagdollSettings`          | `Ragdoll.h`                  | Concrete `Skeleton` is used instead of a `ISkeleton` interface. RagdollSettings stores a direct pointer to a Skeleton object, which means `RagdollSettings` is hard-wired to one specific implementation of a skeleton. |
| 2 | `PhysicsSystem`     | `PhysicsSystem.h`     | `BroadPhaseQuadTree` is a concrete implementation, not an Interface. The broadphase is the system that quickly filters which objects might be colliding. PhysicsSystem hardcodes it as BroadPhaseQuadTree. If a different broadphase algorithm (i.e. a grid or a BVH tree) needs to be implemented, the PhysicsSystem will also need to change.       |
| 3 | `Hair`                     | `Hair/` internals            | Concrete `SoftBodySharedSettings` is used with no abstraction. The Hair simulation internally depends directly on`SoftBodySharedSettings` so if the soft body settings structure ever changes `Hair` must change too, even if Hair's own behaviour has not changed.                                       |
| 4 | `ContactConstraintManager` | `ContactConstraintManager.h` | Concrete `Body` is used instead of a `IBody` interface. `ContactConstraintManager` takes direct references to `Body` objects, which means the constraint manager is tightly coupled to the full `Body` implementation. It is not currently possible to test the constraint manager with a lightweight mock body, but an actual one must be set up with all its dependencies. |
